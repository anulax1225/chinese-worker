...
import { update } from '@/actions/App/Http/Controllers/Api/V1/SystemPromptController';
...
onMounted(() => {
    form.value = {
        ...
        required_variables: props.prompt.required_variables || [],
        default_values: (props.prompt.default_values as Record<string, string>) || {},
        is_active: props.prompt.is_active,
    };
});
...
const submit = async () => {
    ...
    try {
        const response = await fetch(update.url(props.prompt.id), {
            method: 'PUT',
            headers: {
        ...
        if (response.ok) {
            router.visit(`/system-prompts/${props.prompt.id}`);
...
<template>
    <AppLayout :title="`Edit ${prompt.name}`">
        <div class="max-w-3xl mx-auto space-y-6">
            <div class="flex items-center gap-4">
                <Button variant="ghost" size="icon" as-child>
                    <Link :href="`/system-prompts/${prompt.id}`">
                        <ArrowLeft class="h-4 w-4" />
                    </Link>
                </Button>
                <div>
                    <h1 class="text-3xl font-bold">Edit System Prompt</h1>
                    <p class="text-muted-foreground">Update {{ prompt.name }}</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Update the prompt's identity</CardDescription>
                    </CardHeader>
                ...
                <div class="flex justify-end gap-4">
                    <Button variant="outline" type="button" as-child>
                        <Link :href="`/system-prompts/${prompt.id}`">Cancel</Link>
                    </Button>
