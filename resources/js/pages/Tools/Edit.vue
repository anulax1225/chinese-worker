...
const getConfigValue = (key: string, defaultValue: any = '') => {
    if (props.tool.config && typeof props.tool.config === 'object') {
        return (props.tool.config as Record<string, any>)[key] ?? defaultValue;
    }
    return defaultValue;
};
...
const form = ref({
    ...
    config: {
        ...
        method: getConfigValue('method', 'GET') as 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH',
        headers: JSON.stringify(getConfigValue('headers', {}), null, 2),
        code: getConfigValue('code', ''),
...
<template>
    <AuthenticatedLayout :title="`Edit ${tool.name}`" :auth="auth">
        <div class="space-y-6">
            <!-- Page Header -->
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Edit Tool</h1>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Update the tool's name and type</CardDescription>
                    </CardHeader>
