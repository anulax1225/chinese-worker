...
<template>
    <div class="flex gap-3">
        <div class="h-8 w-8 shrink-0 rounded-full bg-indigo-500/10 flex items-center justify-center">
            <FileText class="h-4 w-4 text-indigo-500" />
        </div>
        <Card class="flex-1 max-w-[85%] p-3 border-indigo-500/30 bg-indigo-500/5">
            <!-- Header -->
            <div class="flex items-center justify-between gap-2 mb-2">
                <div class="flex items-center gap-2">
                    <Badge variant="outline" class="bg-indigo-500/10 text-indigo-600 border-indigo-500/30 text-xs">
                        {{ toolName || 'document' }}
                    </Badge>
                    <span class="text-xs text-muted-foreground">Document</span>
                </div>
                <button
                    type="button"
