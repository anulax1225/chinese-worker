...
class ToolExecutor:
    ...
    async def execute(
        ...
        self._msg(f"[#f9e2af]Executing tool:[/#f9e2af] {tool_name}")

        if tool_name not in self.tools:
            error_msg = f"Unknown tool: {tool_name}"
            self._msg(f"[#f38ba8]{error_msg}[/#f38ba8]")
        ...
        try:
            ...
            if output:
                ...
                self._msg(f"[#7f849c]Output: {preview}[/#7f849c]")
            if error:
                self._msg(f"[#f38ba8]Error: {error[:100]}[/#f38ba8]")
            ...
            error_msg = str(e)
            self._msg(f"[#f38ba8]Tool execution failed: {error_msg}[/#f38ba8]")
            await self._submit_result(conversation_id, call_id, False, None, f"[Tool failed: {error_msg}]")
            return False, None
    ...
    async def reject(self, conversation_id: int, tool_request: Dict[str, Any]) -> None:
        call_id = tool_request.get("call_id")
        if call_id:
            self._msg("[#f9e2af]Tool execution rejected.[/#f9e2af]")
    ...
    async def _submit_result(
        ...
        try:
            ...
            self._msg(f"[#f38ba8]Failed to submit tool result: {e}[/#f38ba8]")
